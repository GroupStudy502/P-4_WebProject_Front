...
import com.jmt.global.exceptions.RestaurantNotFoundException;
...
public class RestaurantInfoService {
    ...
    public ListData<Restaurant> getList(RestaurantSearch search) {
        int page = Math.max(search.getPage(), 1); //1페이지부터 조회가 될 수 있도록
        int limit= search.getLimit(); // 한 페이지당 보여줄 레코드 갯수
